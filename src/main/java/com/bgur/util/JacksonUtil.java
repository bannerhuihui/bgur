...
public class JacksonUtil {
     ...
     * 默认日期格式化对象
     */
    private static ThreadLocal<SimpleDateFormat> formatLocal =
    ...
    public static String getString(JsonNode jsonNode, String key, String defaultValue) {
        ...
        return Optional.ofNullable(jsonNode.get(key)).filter(x -> !x.isNull()).map(JacksonUtil::getJsonNodeString)
            .orElse(defaultValue);
    }
    ...
    public static Date getDate(JsonNode jsonNode, String key, SimpleDateFormat format) {
        ...
        String dateStr = Optional.ofNullable(jsonNode.get(key)).filter(x -> !x.isNull())
            .map(JacksonUtil::getJsonNodeString).orElse(null);
        if (dateStr == null) {
            return null;
        }
        try {
