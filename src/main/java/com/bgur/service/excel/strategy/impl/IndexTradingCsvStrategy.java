 ...
 * @projectName: bgur
 * @package: com.bgur.service.excel.strategy.impl
 * @className: IndexTradingCsvStrategy
 * @author: huihui
 * @description: 指数交易CSV解析策略
 * @date: 2025/10/21 23:30
 * @version: 1.0
 */
@Component
public class IndexTradingCsvStrategy implements CsvParseStrategy {
    ...
    @Override
    public List<IndexTrading> parse(ExcelBean excelBean) throws Exception {
        CsvReader reader = CsvUtil.getReader();
        InputStreamReader inputStreamReader = new InputStreamReader(
                excelBean.getExcelFile().getInputStream(),
                Charset.forName("GBK")
        );
        return reader.read(inputStreamReader, IndexTrading.class);
    }
    ...
    public CommonResult execute(ExcelBean excelBean) {
        ...
        List<IndexTrading> buffer = new ArrayList<>(BATCH_SIZE);
        ...
        try (InputStreamReader inputStreamReader = new InputStreamReader(
            ...
            reader.read(inputStreamReader, (CsvRow row) -> {
                ...
                try {
                    IndexTrading bean = row.toBean(IndexTrading.class);
                    buffer.add(bean);
    ...
    private void flushBatch(List<IndexTrading> batch) {
        BulkOperations ops = mongoTemplate.bulkOps(BulkOperations.BulkMode.UNORDERED, IndexTrading.class);
        ops.insert(batch);
        ops.execute();
    }
}